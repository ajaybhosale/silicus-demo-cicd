...
var nonSorting = [4, 5, 4];
$(document).ready(function () {

    var records;
    var url = siteUrl + "/admin/aclpermissions";
    var urlRecordList = siteUrl + "admin/aclpermissions/getList";
    var recordsColumn = [
        {"data": "role"},
        {"data": "module"},
        {"data": "action"},
    ...
    $(document).on('click', '#btnAdd', function () {
        ...
        $('#btn-save').val("add");
        $('#frmAclpermissions').trigger("reset");
        $('#myModal').modal('show');
    });
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                else
                {
                    ...
                    $('div#modalDetail').find('#module').html(data.module);
                    $('div#modalDetail').find('#action').html(data.action);
    ...
    $("#btn-save").click(function (e) {
        if ($('form#frmAclpermissions').validate()) {
            $.ajaxSetup({
                headers: {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    {
                        $('#permissionGrid').DataTable().ajax.reload(); //to refresh datatables
                        $("#myModal .close").click();
    ...
    $.fn.updateRecords = function () {
        ...
        $('#permissionGrid tbody').on('click', '.clsDelete', function () {
