...
function delJobCategory(id) {
    url = siteUrl + '/admin/job/category/delete/' + id;
...
function validataJobCategoryForm() {
    var form = document.getElementById("jobCategoryForm");
    var categoryName = $('#categoryName').val();
    form.onsubmit = function () {
        if (categoryName == '') {
            $('#categoryName_error').show().html('Please enter job category');
            return false;
        } else {
...
$(document).ready(function () {
    var records;
    var urlRecordList = siteUrl + "admin/job/category/getCategoryList";
    var urlRecordEdit = siteUrl + "admin/pages/updatePageStatus";
    var urlDelete = siteUrl + "admin/job/category/delete";
    var recordsColumn = [
        {"data": "category_name"},
            ...
            BootstrapDialog.confirm({
                ...
                callback: function (result) {
                    if (result) {
                        ...
                        BootstrapDialog.show({message: 'Job category deleted successfully!'});
                    }
                }
            });
        });
        ...
        "columnDefs": [{
                ...
                "render": function (data, type, row, meta) {
                    if (type === 'display') {
                        ...
                        if (meta.col == 2) {
                            data = "<a class='inline-link clsEdit glyphicon glyphicon-pencil' href='" + siteUrl + 'admin/job/category/edit/' + row.edit + "' eid='" + row.edit + "'></a>";
                        }
